...
count differs between renders → React throws *"Rendered more hooks than during the
previous render."* ESLint flags this as `react-hooks/rules-of-hooks` (error).
...
pre-commit hook keyed on lint is either broken or disabled, which is *why* the
...
*Audit scope: static analysis + full source read. Dynamic/runtime profiling,

...
it authored by *other* users) were interpolated directly between
...
- ⚠️ *Rec:* replace import-time resolver registration with explicit/lazy init (see
  C2 follow-up).
- ⚠️ *Rec:* `AIEngine` header comment says calls "throw not_implemented" — stale
...
- ⚠️ *Rec:* provider **tiers** (`fast|balanced|deep`) exist on model descriptors
...
- ⚠️ *Rec:* `AIGenerateParams.signal` (abort) is threaded but no adapter honors
...
- ⚠️ *Rec:* `buildMessages` spreads the **full** history with no windowing,
...
- ⚠️ *Known/documented:* `localStorage` obfuscation is **not** encryption; the code
...
- ⚠️ *Rec:* markdown links in chat render with `target="_blank"` + `rel="noreferrer"`
...
- ⚠️ *Rec:* `store.appendDelta` rebuilds the conversations array **and** the
...
- ⚠️ *Rec:* `AIProviderId` includes `local` and `mock`, but `MODEL_CATALOG.local`
...
*Overall: a solid, extensible, type-safe AI module. The two critical fixes close a
...
moving orchestration server-side (`AI_ARCHITECTURE.md`).*

...
(`useSyncExternalStore` facade) that is _either_ localStorage-only (mock) or **hydrated
...
**Gap (not blocking login):** HR invitation _issuance_ is a localStorage mock (§4) — a "sent"
...
- **Missing:** Employee **mutations** go through `hr/employees-store.ts` — a "localStorage-backed
  reactive _overlay_" (`employees-store.ts:2`), not the backend. Invitations to add employees are
...
Drift guarded by tests. **Backend:** yes. **Mock:** no. _(Non-blocking integrity notes:
some RLS write policies are column-unrestricted — see PRODUCTION_GAP_ANALYSIS H1.)_
...
_(Note: attendance UI/hooks query Supabase directly, bypassing a repository — architecture
smell, not a functional gap. Self-writable metrics integrity hole exists — see gap analysis.)_
...
day-1 compromise; missing multi-user _core_ collaboration is not.
...
1. **Provision Supabase** and apply all 13 migrations (project ref `mgbtonsatvffknsjdxfe`
...
4. **Create the 9 other employees.** Because invite _issuance_ is mock (§4), the reliable path
   ...
   _(This is the single biggest onboarding friction — see quick-win Q1.)_

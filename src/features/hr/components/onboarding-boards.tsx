    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Badge variant="outline">
          {done}/{tasks.length} completed · {pct}%
        </Badge>
      </div>

      {!employee ? (
        <EmptyState title="No onboarding in progress" icon={GraduationCap} />
      ) : (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <CardTitle className="flex items-center gap-2 text-sm">
              <EmployeeAvatar employee={employee} size={28} /> {employee.name}
            </CardTitle>
...
export function OffboardingBoard() {
  ...
  return (
    <div className="space-y-4">
      {!employee ? (
        <EmptyState title="No active offboarding" icon={Plane} />
      ) : (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <CardTitle className="flex items-center gap-2 text-sm">
              <EmployeeAvatar employee={employee} size={28} /> {employee.name} · exits{" "}
              {new Date(tasks[0].exitDate).toLocaleDateString()}
            </CardTitle>
            <Badge variant="outline">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Recently offboarded</CardTitle>
        </CardHeader>
        <CardContent>
          {employees.filter((e) => e.status === "deactivated").length === 0 ? (
            <EmptyState title="No archived employees" />
          ) : (

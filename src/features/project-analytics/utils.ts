...
export function snapshotTasks(tasks: Task[]): ProjectTasksSnapshot {
  ...
  const overdue = tasks.filter(
    (t) =>
      t.dueDate && new Date(t.dueDate).getTime() < now && !["done", "cancelled"].includes(t.status),
  ).length;
...
export function tasksPerUser(tasks: Task[], logs: TimeLog[]): UserStat[] {
  ...
  for (const t of tasks) {
    ...
    if (!map.has(id)) {
      map.set(id, {
        userId: id,
        name: id === "unassigned" ? "Unassigned" : employeeName(id),
        total: 0,
        done: 0,
        open: 0,
        overdue: 0,
    ...
    if (t.dueDate && new Date(t.dueDate).getTime() < Date.now() && t.status !== "done")
      s.overdue += 1;
  }
...
export function sprintVelocityMock(projectId: string) {
  return sprintsForProject(projectId)
    .slice(-6)
    .map((sp, i) => ({
      label: sp.name,
...
export function burndownMock(projectId: string) {
  const sprint =
    sprintsForProject(projectId).find((s) => s.status === "active") ??
    sprintsForProject(projectId)[0];
  ...
  for (let i = 0; i <= totalDays; i++) {
    ...
    const actual = Math.max(
      0,
      Math.round(capacity - (capacity * i) / totalDays + noise + (i > totalDays / 2 ? 2 : 0)),
    );
...
export function avgCycleTime(tasks: Task[]): number {
  ...
  return (
    Math.round(
      (3 +
        (tasks.length % 7) +
        ((0.4 * tasks.filter((t) => t.status === "done").length) / Math.max(1, tasks.length)) *
          10) *
        10,
    ) / 10
  );
...
export function unifiedActivity(projectId: string, limit = 40): AnalyticsActivityItem[] {
  ...
  for (const sp of sprintsForProject(projectId)) {
    out.push({
      ...
      at: sp.createdAt,
      kind: "sprint",
      summary: `Sprint created — ${sp.name}`,
      actorId: undefined,
    });
  }

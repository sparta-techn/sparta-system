...
import { Activity, AlertTriangle, CheckCircle2, Clock, FileText, Flame, GaugeCircle, MessageSquare,
...
import {
  ...
  avgCycleTime, avgHoursPerTask, bottleneckStage, burndownMock, cumulativeCompletion,
  dependencyInsights, employeeName, filterProjectTasks, projectTimeLogs, snapshotTasks,
  sprintProgressList, sprintVelocityMock, sprintsForProject, statusDistribution,
  tasksCompletedPerDay, tasksCreatedPerDay, tasksPerUser, topTimeConsumingTasks, totalHours,
  unifiedActivity,
} from "../utils";
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="flex flex-wrap items-center gap-2 p-3">
          ...
          <Select value={userId} onValueChange={setUserId}>
            ...
            <SelectContent>
              ...
              {employees.slice(0, 24).map((e) => <SelectItem key={e.id} value={e.id}>{e.name}</SelectItem>)}
            </SelectContent>
          </Select>
          ...
          <Badge variant="outline" className="ml-auto gap-1"><Sparkles className="size-3" /> Read-only</Badge>
        </CardContent>
      </Card>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <StatCard label="Completed" value={data.snap.completed} icon={TrendingUp}
          hint={`${data.snap.completionPct}% complete`} />
        <StatCard label="Open" value={data.snap.open} icon={Activity} />
        <StatCard label="Overdue" value={data.snap.overdue} icon={AlertTriangle}
          trend={data.snap.overdue > 0 ? { direction: "up", value: "Needs attention", intent: "negative" } : undefined} />
        <StatCard label="Blocked" value={data.snap.blocked} icon={Flame}
          trend={data.snap.blocked > 0 ? { direction: "up", value: "In dependency", intent: "negative" } : undefined} />
      </div>
          ...
          <CardContent className="grid gap-2 sm:grid-cols-2">
            {insights.map((i) => (
              <div key={i.id} className={cn(
                ...
                {i.description ? <p className="mt-0.5 text-xs text-muted-foreground">{i.description}</p> : null}
              </div>
            ))}
          </CardContent>
        </Card>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Cumulative completion</CardTitle></CardHeader>
          <CardContent>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Tasks created per day</CardTitle></CardHeader>
          <CardContent><LineChart data={data.created} ariaLabel="Tasks created" /></CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Tasks completed per day</CardTitle></CardHeader>
          <CardContent>
            <LineChart data={data.completed} colorClass="stroke-success" ariaLabel="Tasks completed" />
          </CardContent>
        </Card>
      </div>

      <div className="grid gap-3 sm:grid-cols-3">
        <StatCard label="Bottleneck stage" value={data.bottleneck} icon={Activity} hint="Stage with most in-flight tasks" />
        <StatCard label="Avg cycle time" value={`${data.cycle}d`} icon={Clock} hint="Mock — from create → done" />
        <StatCard label="Throughput (range)" value={data.completed.reduce((a, b) => a + b.value, 0)} icon={TrendingUp}
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {data.perUser.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">No assigned tasks in this view.</p>
            ...
            return (
              <div key={u.userId} className="grid grid-cols-[1fr_2fr_auto] items-center gap-3">
                <div className="flex min-w-0 items-center gap-2">
                  <Avatar className="size-7"><AvatarFallback>{u.name.slice(0, 2).toUpperCase()}</AvatarFallback></Avatar>
                  <span className="truncate text-sm font-medium">{u.name}</span>
                </div>
                <div className="space-y-1">
                  <Progress value={pct} className="h-2" />
                  <p className="text-xs text-muted-foreground">{u.done}/{u.total} done · {u.open} open · {u.overdue} overdue</p>
                </div>
                <span className="tabular-nums text-xs text-muted-foreground">{u.hours.toFixed(1)}h</span>
              </div>
            );
          })}
        </CardContent>
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Workload distribution</CardTitle></CardHeader>
          <CardContent>
            <DonutChart
              data={data.perUser.slice(0, 6).map((u) => ({ label: u.name, value: u.total }))}
              centerLabel="Tasks" centerValue={String(data.snap.total)}
              ariaLabel="Tasks per user"
            />
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Contribution (done)</CardTitle></CardHeader>
          <CardContent>
            <BarChart data={data.perUser.slice(0, 8).map((u) => ({ label: u.name.split(" ")[0], value: u.done }))}
              colorClasses={["fill-success"]} ariaLabel="Completed tasks per user" />
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <StatCard label="Tracked tasks" value={new Set(data.logs.map((l) => l.taskId)).size} icon={Activity} />
      </div>

      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-base">Top time-consuming tasks</CardTitle></CardHeader>
        <CardContent>
            ...
            <ul className="divide-y">
              {data.topTime.map((t) => (
                <li key={t.taskId} className="flex items-center justify-between gap-3 py-2 text-sm">
                  <Link to="/app/tasks/$id" params={{ id: t.taskId }} className="min-w-0 flex-1 truncate hover:underline">
                    {t.title}
                  </Link>
      ...
      {projectSprints.length > 0 ? (
        <div className="grid gap-4 xl:grid-cols-2">
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Sprint velocity (mock)</CardTitle></CardHeader>
            <CardContent>
              <BarChart data={data.velocity} ariaLabel="Story points per sprint" />
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Sprint completion</CardTitle></CardHeader>
            <CardContent className="space-y-2">
              {data.sprints.map((sp) => (
                <div key={sp.sprint.id} className="space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-medium">{sp.sprint.name}</span>
                    <span className="text-muted-foreground tabular-nums">{sp.done}/{sp.total} · {sp.pct}%</span>
                  </div>
          ...
          <Card className="xl:col-span-2">
            <CardHeader className="pb-2"><CardTitle className="text-base">Burndown (mock — active sprint)</CardTitle></CardHeader>
            <CardContent>
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_2fr]">
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Dependency insights</CardTitle></CardHeader>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Dependency chain (visual)</CardTitle></CardHeader>
          <CardContent>
            <DependencyChain blockers={data.tasks.filter((t) => t.status === "blocked").slice(0, 4)} />
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {data.timeline.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">No activity in this project yet.</p>
          ) : (
              ...
              {data.timeline.map((a) => (
                <li key={a.id} className="relative">
                  <span className="absolute -left-[27px] grid size-5 place-items-center rounded-full border bg-background">
                    {a.kind === "comment" ? <MessageSquare className="size-3" /> :
                     a.kind === "file" ? <FileText className="size-3" /> :
                     a.kind === "sprint" ? <Flame className="size-3" /> :
                     <Activity className="size-3" />}
                  </span>
                  <div className="flex flex-wrap items-baseline justify-between gap-2 text-sm">
                    <p className="min-w-0 flex-1 truncate">
                      {a.actorId ? <span className="font-medium">{employeeName(a.actorId)} · </span> : null}
                      {a.summary}
                    </p>
...
function HealthCard({ score, level, factors }:
  { score: number; level: HealthLevel; factors: { label: string; value: number; weight: number }[] }) {
    ...
    <Card>
      ...
      <CardContent className="flex flex-col items-center gap-4 sm:flex-row sm:items-start">
        <svg viewBox="0 0 128 128" className="size-32 shrink-0">
          ...
          <circle cx="64" cy="64" r={r} className={cn("fill-none transition-all", c.ring)}
            strokeWidth="10" strokeLinecap="round"
            strokeDasharray={`${dash} ${circ - dash}`} transform="rotate(-90 64 64)" />
          <text x="64" y="60" textAnchor="middle" className="fill-foreground text-2xl font-semibold">{score}</text>
          <text x="64" y="80" textAnchor="middle" className={cn("text-xs font-medium", c.tone)}>{c.label}</text>
        </svg>
          ...
          {factors.map((f) => (
            <li key={f.label} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="tabular-nums">{Math.round(f.value)} · w{Math.round(f.weight * 100)}%</span>
              </div>
...
function BurndownChart({ data }: { data: { label: string; value: number; ideal: number }[] }) {
  ...
  const w = 600, h = 220, pad = { t: 12, r: 12, b: 24, l: 32 };
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className="h-auto w-full" role="img" aria-label="Burndown chart">
      <polyline fill="none" className="stroke-muted-foreground/40" strokeWidth="2" strokeDasharray="4 4" points={ideal} />
      ...
      {data.map((d, i) => (
        <g key={i}>
          ...
          <text x={x(i)} y={h - 6} textAnchor="middle" className="fill-muted-foreground text-[10px]">{d.label}</text>
        </g>
      ))}
    </svg>
  );
...
function DependencyChain({ blockers }: { blockers: { id: string; title: string; ref: string }[] }) {
  if (blockers.length === 0) {
    return <p className="text-sm text-muted-foreground">No blocked tasks. Dependency graph is clear.</p>;
  }
  return (
    <div className="flex flex-wrap items-center gap-2 text-xs">
      {blockers.map((b, i) => (
        <div key={b.id} className="flex items-center gap-2">
          <Link to="/app/tasks/$id" params={{ id: b.id }}

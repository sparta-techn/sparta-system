...
export function AttendanceBadge({ status, size }: { status: AttendanceStatus; size?: "sm" | "md" | "lg" }) {
  const meta = ATTENDANCE_STATUS_META[status];
  return <StatusBadge tone={meta.tone} label={meta.label} size={size} />;
}

export function SessionStatusBadge({ status, size }: { status: WorkSessionStatus; size?: "sm" | "md" | "lg" }) {

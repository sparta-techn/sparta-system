...
export function TeamStatusBoard({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  const rows = useMemo(() => {
    const filtered = managerEmployees.filter((e) => {
      ...
      if (
        q &&
        !`${e.name} ${e.role} ${e.department} ${e.currentTask ?? ""}`
          .toLowerCase()
    ...
    <Card>
      <CardHeader className="gap-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Live view across the team. Search, filter, and act fast.
            </CardDescription>
          </div>
        </div>
        <div className="grid gap-2 sm:grid-cols-[1fr_auto_auto]">
          ...
          >
            ...
            <SelectContent>
              ...
              {["Engineering", "Design", "Product", "Data", "QA", "DevOps", "Marketing"].map(
                (d) => (
      ...
      <CardContent className="p-0">
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <Th onClick={() => toggleSort("workSeconds")} className="text-right">
                    Work
                  </Th>
                  ...
                  <TableRow key={e.id} className="hover:bg-accent/30">
                    ...
                    <TableCell className="max-w-[240px]">
                      <p className="truncate text-sm text-foreground">
                        {e.currentTask ?? <span className="text-muted-foreground">—</span>}
                      </p>
                    </TableCell>
                    <TableCell className="text-right text-sm tabular-nums">
                      {formatHrs(e.workSeconds)}
                    </TableCell>
                    <TableCell className="text-right text-sm tabular-nums text-muted-foreground">
                      {formatHrs(e.breakSeconds)}
                    </TableCell>
                    ...
                    <TableCell>
                      <StatusBadge
                        tone={HEALTH_TONE[e.workHealth]}
                        label={HEALTH_LABEL[e.workHealth]}
                        size="sm"
                        withDot
                      />
                    </TableCell>
                    <TableCell
                      className={cn(
                        ...
                        e.openDependencies > 2 && "text-warning font-semibold",
                      )}
                    >
                      {e.openDependencies}
                    </TableCell>
        ...
        <div className="flex items-center justify-between border-t border-border px-4 py-3 text-xs text-muted-foreground">
          <span>
            {rows.length} employees · page {page} of {totalPages}
          </span>
          <div className="flex items-center gap-1">
            <Button
              variant="ghost"

...
function rate(key: "checkin" | "midday" | "eod") {
  ...
  return { done, total: eligible.length, pct: Math.round((done / Math.max(1, eligible.length)) * 100) };
}
...
export function ReportCompliance() {
  ...
  const eod = rate("eod");
  const missing = managerEmployees.filter((e) => Object.values(e.reports).includes("missed")).length;
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 pt-2 text-sm">
          <div className="rounded-lg border border-border bg-surface/40 px-3 py-2">
            ...
            <p className="font-display text-xl font-semibold tabular-nums text-destructive">{missing}</p>
          </div>
          <div className="rounded-lg border border-border bg-surface/40 px-3 py-2">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Late / Pending</p>
...
function Row({ label, data }: { label: string; data: { done: number; total: number; pct: number } }) {
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-sm">
        <span className="text-foreground">{label}</span>
        <span className="tabular-nums text-muted-foreground">{data.done}/{data.total} · {data.pct}%</span>
      </div>

...
function CompactList({
  ...
  if (!tasks.length) {
    return <p className="rounded border border-dashed bg-card/50 p-4 text-center text-xs text-muted-foreground">Nothing here right now.</p>;
  }
        ...
        return (
          <li key={t.id} className="rounded border bg-card px-2.5 py-2">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-[11px] text-muted-foreground">
              ...
              {project ? <span>{project.icon} {project.name}</span> : null}
              {t.dueDate ? (
...
export function MyTasksWidget() {
  const filters: TaskFilters = { assigneeIds: [CURRENT_USER_ID], status: ["todo", "in_progress", "review", "qa"] };
...
export function TodayTasksWidget() {
  const tasks = useTasksState((s) => {
    ...
    return s.tasks
      .filter((t) => {
        if (!t.dueDate || t.deletedAt || t.status === "done" || t.status === "cancelled") return false;
  ...
  return (
    <WidgetShell title="Today's tasks" href="/app/tasks/all" count={tasks.length}>
      {tasks.length === 0 ? (
        ...
        <ul className="space-y-1.5">
          {tasks.map((t) => (
            <li key={t.id} className="flex items-center gap-2 rounded border bg-card px-2.5 py-2 text-sm">
              <Link
                to="/app/tasks/$id"
                params={{ id: t.id }}
